                    ...
                    >
                            ...
                            <div className="space-y-8">
                                    ...
                                    <div className="p-10 bg-rose-500/5 rounded-[3rem] border border-rose-500/10 animate-fade-in">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {rewrittenResume.skillGapAnalysis.map((gap, i) => (
                                                <span key={i} className="px-5 py-2.5 bg-[var(--bg-surface)] rounded-xl text-xs font-bold text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-500/20 shadow-sm uppercase tracking-tight">/ {gap}</span>
                                            ))}
                                        </div>
                                    </div>
                                )}
                                ...
                                {rewrittenResume?.rewrittenBullets?.map((section, idx) => (
                                    <div key={idx} className="grid md:grid-cols-2 gap-8 relative p-2">
                                        ...
                                        <div className="bg-[var(--bg-surface)] p-8 rounded-[2.5rem] border-2 border-primary-100 dark:border-primary-800/40 shadow-xl relative group">
                                            ...
                                            <span className="text-xs font-black text-primary-600 dark:text-primary-400 uppercase tracking-widest mb-6 block">AI Executive Strategy</span>
                                            ...
                                            <div className="mt-8 pt-6 border-t border-[var(--border-primary)] flex items-start gap-3">
                                                <div className="p-1.5 bg-emerald-50 dark:bg-emerald-950/40 rounded-lg text-emerald-600 dark:text-emerald-400"><CheckCircle size={14} /></div>
                                                <p className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">Rationale: <span className="text-[var(--text-secondary)] normal-case font-bold">{section.reasoning}</span></p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        {activeSubTab === 'tailor' && (
                            <div className="space-y-8">
                                                    ...
                                                    <div className="flex gap-4">
                                                        {Object.entries(tailorData.skillRelevance || {}).map(([key, val]) => (
                                                            <div key={key} className="text-center">
                                                                <p className="text-xs font-bold text-[var(--text-muted)] uppercase">{key}</p>
                                                                <p className="text-xs font-black text-emerald-700 dark:text-emerald-400">{val}%</p>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                                            ...
                                                            {tailorData.keywordAnalysis?.map((kw, i) => (
                                                                <div key={i} className="p-4 bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-primary)] flex items-center justify-between group hover:border-emerald-200 transition-colors">
                                                                    <div className="flex flex-col">
                                                                        ...
                                                                        <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-tighter">Imp: {kw.jdImportance}</span>
                                                                    </div>
                                                                    <div className={`text-xs font-black ${kw.resumeDensity > 50 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'}`}>{kw.resumeDensity}%</div>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <div className="flex flex-wrap gap-2">
                                                            {(tailorData.missingSkills || []).map((s, i) => (
                                                                <span key={i} className="px-3 py-1.5 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-bold rounded-lg border border-rose-100 dark:border-rose-500/20 uppercase italic">! {s}</span>
                                                            ))}
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                {tailorData && (
                                    <div className="bg-[var(--bg-surface)] p-12 rounded-[4rem] border-2 border-emerald-100 dark:border-emerald-500/20 shadow-xl animate-fade-in divide-y divide-slate-100 dark:divide-[var(--border-primary)]">
                                        <div className="pb-8">
                        ...
                        {activeSubTab === 'cover-letter' && (
                            <div className="space-y-8 animate-fade-in px-4">
                                <div className="bg-[var(--bg-surface)] p-12 rounded-[3.5rem] border border-[var(--border-primary)] shadow-sm text-center">
                                    <div className="w-20 h-20 bg-cyan-100 dark:bg-cyan-500/10 rounded-[2rem] flex items-center justify-center text-cyan-600 dark:text-cyan-400 mx-auto mb-6"><FileText size={32} /></div>
                                    ...
                                    <button onClick={() => runFeature('cover-letter')} disabled={loading} className="btn-primary bg-cyan-600 hover:bg-cyan-700 !px-16 !py-5 shadow-cyan-100 dark:shadow-none">
                                        {loading ? <Loader2 className="animate-spin" /> : <><Sparkles size={20} /> Generate Professional Letter</>}
                                    </button>
                                </div>

                                {coverLetter?.coverLetter && (
                                    <div className="md:col-span-2 p-12 bg-cyan-50 dark:bg-cyan-950/20 rounded-[4rem] text-[var(--text-primary)] shadow-xl border border-cyan-100 dark:border-cyan-800/40 relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 rounded-full translate-x-10 -translate-y-10 blur-3xl" />
                                        <div className="flex flex-col md:flex-row justify-between md:items-center gap-6 mb-10 pb-6 border-b border-cyan-200 dark:border-cyan-800/40 uppercase tracking-[0.2em] font-bold text-xs">
                                            <div className="flex items-center gap-3 text-cyan-600 dark:text-cyan-400">
                                                <div className="w-10 h-10 bg-[var(--bg-surface)] rounded-xl flex items-center justify-center border border-cyan-100 dark:border-cyan-800/40 shadow-sm"><UserCheck size={20} /></div>
                                                Master Draft: {selectedRole}
                                            </div>
                                            <button onClick={() => handleCopy(coverLetter.coverLetter, 'cl')} className="flex items-center gap-2 group/btn text-cyan-600 dark:text-cyan-400 hover:opacity-70 transition-opacity">
                                        ...
                                        <div className="mt-12 flex justify-between items-center bg-[var(--bg-surface)] p-6 rounded-3xl border border-cyan-100 dark:border-cyan-800/40 shadow-sm">
                                            <p className="text-xs font-bold text-[var(--text-muted)] tracking-widest uppercase">Tone Setting: <span className="text-cyan-600 dark:text-cyan-400">{coverLetter.toneAnalysis}</span></p>
                                            <button onClick={() => downloadTextFile(coverLetter.coverLetter, `CoverLetter-${candidateName}.txt`)} className="px-10 py-4 bg-cyan-600 hover:bg-cyan-700 text-white rounded-2xl text-xs font-bold transition-all flex items-center gap-2 uppercase tracking-widest shadow-lg shadow-cyan-100 dark:shadow-none"><Download size={14} /> Download PDF Ready</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-10 animate-fade-in px-4 pb-20">
                                ...
                                {linkedinData && (
                                    <div className="grid lg:grid-cols-3 gap-8">
                                        <div className="lg:col-span-2 space-y-8">
                                            <div className="bg-[var(--bg-surface)] p-10 rounded-[3rem] border border-[var(--border-primary)] shadow-sm relative group overflow-hidden">
                                                ...
                                                <div className="flex items-center gap-4 mb-8">
                                                    <span className="text-xs font-black text-primary-600 dark:text-primary-300 bg-primary-50 dark:bg-primary-950/40 border border-primary-200 dark:border-primary-800/40 px-4 py-2 rounded-full uppercase tracking-widest">Optimized High-Impact Headline</span>
                                                ...
                                                <button onClick={() => handleCopy(linkedinData.headline, 'li_h')} className="text-xs font-black text-cyan-600 dark:text-cyan-400 flex items-center gap-2 hover:opacity-70 group/copy">
                                            ...
                                            <div className="bg-[var(--bg-surface)] p-10 rounded-[3.5rem] border border-[var(--border-primary)] shadow-sm">
                                                <div className="flex justify-between items-center mb-10 pb-6 border-b border-[var(--border-primary)]">
                                                    <span className="text-xs font-black text-cyan-600 dark:text-cyan-400 uppercase tracking-widest">Elite "About" Narrative</span>
                                                    <button onClick={() => handleCopy(linkedinData.about, 'li_a')} className="flex items-center gap-2 text-xs font-black text-[var(--text-muted)] hover:text-cyan-600 dark:hover:text-cyan-400 uppercase tracking-widest transition-colors">
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    {linkedinData.experience.map((exp, i) => (
                                                        <div key={i} className="p-8 bg-[var(--bg-surface)] rounded-[2.5rem] border border-[var(--border-primary)] shadow-sm group">
                                                            <h5 className="text-xs font-black text-primary-600 dark:text-primary-400 uppercase mb-4">{exp.company}</h5>
                                        ...
                                        <div className="space-y-8">
                                            <div className="bg-cyan-50 dark:bg-cyan-950/20 p-8 rounded-[3rem] text-[var(--text-primary)] shadow-xl border border-cyan-100 dark:border-cyan-800/40 relative overflow-hidden group">
                                                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cyan-400 via-blue-400 to-sky-400" />
                                                <div className="flex items-center gap-3 mb-6 text-cyan-600 dark:text-cyan-400">
                                                    <ImageIcon size={20} />
                                            ...
                                            <div className="bg-[var(--bg-surface)] p-8 rounded-[3rem] border border-[var(--border-primary)] shadow-sm">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {linkedinData.skillsToPin?.map((s, i) => (
                                                        <span key={i} className="px-3 py-1.5 bg-cyan-50 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 text-xs font-bold rounded-xl border border-cyan-100 dark:border-cyan-800/40 uppercase italic"># {s}</span>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-8">
                                        {emailData.templates.map((tmpl, idx) => (
                                            <div key={idx} className="bg-[var(--bg-surface)] p-10 rounded-[3.5rem] border border-[var(--border-primary)] shadow-sm flex flex-col h-full hover:shadow-xl transition-all relative group">
                                                ...
                                                <span className="text-xs font-black text-primary-600 dark:text-primary-300 bg-primary-50 dark:bg-primary-950/40 border border-primary-200 dark:border-primary-800/40 px-4 py-2 rounded-full uppercase tracking-[0.2em] mb-8 self-start">{tmpl.type}</span>
                                                ...
                                                <button onClick={() => handleCopy(tmpl.body, `em_${idx}`)} className="w-full py-5 bg-cyan-600 text-white hover:bg-cyan-700 rounded-3xl text-sm font-bold transition-all flex items-center justify-center gap-3 uppercase tracking-widest active:scale-95 shadow-lg shadow-cyan-100 dark:shadow-none">
