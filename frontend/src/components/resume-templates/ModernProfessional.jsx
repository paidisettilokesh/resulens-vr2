        ...
        <div className="font-sans text-slate-900 leading-relaxed bg-white text-black p-12 mx-auto shadow-sm w-full max-w-[850px] min-h-[1100px]">
            <div className="flex justify-between items-end mb-4">
                <div>
                    ...
                    <p className="text-xs font-bold text-blue-600 uppercase tracking-widest">{data.experience?.[0]?.role || 'Professional Portfolio'}</p>
                </div>
                <div className="text-right text-xs font-bold text-slate-500 leading-relaxed">
            ...
            <div className="grid grid-cols-[1fr_2.5fr] gap-10">
                <div className="space-y-8">
                    <div>
                        <h4 className="text-xs font-black uppercase tracking-widest text-blue-600 mb-4 border-b border-slate-200 pb-2">Expertise</h4>
                    ...
                    <div>
                        <h4 className="text-xs font-black uppercase tracking-widest text-blue-600 mb-4 border-b border-slate-200 pb-2">Education</h4>
                        {data.education.map(edu => (
                            <div key={edu.id} className="mb-5">
                                <h5 className="text-[11px] font-black text-slate-900">{edu.school}</h5>
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mt-1">{edu.degree}</p>
                ...
                <div className="space-y-8">
                    {data.personal.bio && (
                        <div>
                            <h4 className="text-xs font-black uppercase tracking-widest text-blue-600 mb-3 border-b border-slate-200 pb-2">Profile</h4>
                            <p className="text-[11.5px] leading-relaxed text-slate-700">{data.personal.bio}</p>
                        </div>
                    )}
                    <div>
                        <h4 className="text-xs font-black uppercase tracking-widest text-blue-600 mb-4 border-b border-slate-200 pb-2">Experience</h4>
                            ...
                            {data.experience.map(exp => (
                                <div key={exp.id} className="relative pl-4 border-l-2 border-blue-100">
                                    <div className="flex justify-between items-baseline mb-1">
                                        ...
                                        <span className="text-xs font-bold text-blue-600/80">{exp.period}</span>
                                    </div>
